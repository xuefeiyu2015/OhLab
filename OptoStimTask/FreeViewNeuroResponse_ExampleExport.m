...
function FreeViewNeuroResponse_ExampleExport(Data, StartTrial, EndTrial,ShowFigureFlag,OutputFlag);
...
%subplot(2,1,1)
ax1 = axes('Position',[0.15 0.7 0.8 0.3]);
AverageRegion = nanmean(RelativeTimeMarker);
area(AverageRegion,[size(Raster,1)+5,size(Raster,1)+5],...
    'FaceColor','k','FaceAlpha',.1,'EdgeAlpha',.1,'HandleVisibility','off','LineStyle','none');
...
%
ax2 = axes('Position',[0.15 0.2 0.8 0.5]);
...
%OutputFigureName=sprintf('%s_%s_N%s_C%s_%sCID_%sNID',MonkeyName,string(RecordDateOriginal),string(NeuronNum),string(ChannelNum),string(ChannelID),string(ClusterID));
%For verson2-brief version
OutputFigureName=sprintf('%s_%s_N%s_C%s_%sCID_%sNID_V2',MonkeyName,string(RecordDateOriginal),string(NeuronNum),string(ChannelNum),string(ChannelID),string(ClusterID));
