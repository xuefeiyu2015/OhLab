...
%Calculate latency using raster
ContinousBinC=5;
RespLatencyC=NaN;
Raster_Stim = Raster(StimTrial ,:);
PreStim_SponWindow = [-CompareTimeBin,0];
PreStim_SpikeIn = sum(Raster_Stim >=PreStim_SponWindow(1) & Raster_Stim < PreStim_SponWindow(2),2);
%Raster
index = 1;
for ii = Interval(1):1:round(Interval(2))
    TimeInv_curr = [Interval(1)+ii,min(Interval(1)+ii+CompareTimeBin,round(Interval(2)))];

    SpikeIn_curr = sum(Raster_Stim >=TimeInv_curr(1) & Raster_Stim < TimeInv_curr(2),2);
    ...
    if StimModulation > 0
        ...
        [h,p_c(index)] = ttest(PreStim_SpikeIn,SpikeIn_curr,'tail','left');
    else
        [h,p_c(index)] = ttest(PreStim_SpikeIn,SpikeIn_curr,'tail','right');
    end

    if index >= ContinousBinC
        cri = 1;
        for j = 1:ContinousBinC
            if p_c(index-j+1) >0.05 | isnan(p_c(index))
                cri = 0;
            end
        end
        if cri == 1 & isnan(RespLatencyC)
            RespLatencyC = ii - ContinousBinC+1;
...
title(sprintf("Laser Power : %1.3f mW; Latency: %.1f ms;LatencyC: %.1f ms",unique(stimPower),ResponseLatency,RespLatencyC));
legend(["Sham","Stim"])
box off
...
DataLib={MeanPSTH_Time_Mean,SpikeCountStim_Mean,SpikeCountControl_Mean,ActivationStrength,ResponseLatency,...
    ...
    Latency3,Latency2,RespLatencyC...
    Raster_Stim,Raster_Control,StimulationStrength_norm,...
    ChannelID,SpikeChannel
