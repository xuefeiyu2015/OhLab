function SaccadeStimEffectExample(Data, StartTrial, EndTrial,ShowFigureFlag,OutputFlag);
ECode;
...
%% Compare between different conditions
    ...
    for j = 1:length(UniqueAngle)
        sel = TargetAngle == UniqueAngle(j);8
       ...
       EyeEcc_Stim_loc{j} = sqrt(EyeChannel_X_StimAlign(sel_stim,:).^2+EyeChannel_Y_StimAlign(sel_stim,:).^2);
       
       EyeX_Control_loc{j} = EyeChannel_X_StimAlign(sel_sham,:);
       EyeY_Control_loc{j} = EyeChannel_Y_StimAlign(sel_sham,:);

        EyeEcc_Control_loc{j} = sqrt(EyeChannel_X_StimAlign(sel_sham,:).^2+EyeChannel_Y_StimAlign(sel_sham,:).^2);

       [h,p_loc(j)] = ttest2(RT_stim_loc_period,RT_control_loc_period);
...
if Monkey==1
    ...
    EyeEcc_Stim_RF = EyeEcc_Stim_loc{RF};
    EyeEcc_Control_RF = EyeEcc_Control_loc{RF};
    ...
    EyeEcc_Stim_NonRF = EyeEcc_Stim_loc{NonRF};
    EyeEcc_Control_NonRF = EyeEcc_Control_loc{NonRF};
...
StartNO = StartNO+1;
   ...
   figtitlestr{FigureIndex}='EyeTrace';
   fig{FigureIndex}=PrepareFigure(StartNO+FigureIndex-1,'w',[70,100, 1200,800],'Name',figtitlestr{FigureIndex});

   subplot(1,2,1)
   g = plot(EyeEcc_Stim_RF','-r');
   
   hold on 
   plot(EyeEcc_Control_RF','-k');


   subplot(1,2,2)
   g = plot(EyeEcc_Stim_NonRF','-r');
   
   hold on 
   plot(EyeEcc_Control_NonRF','-k');
