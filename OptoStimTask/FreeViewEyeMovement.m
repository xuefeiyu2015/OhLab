function FreeViewEyeMovement(Data, StartTrial, EndTrial,ShowFigureFlag,OutputFlag);
ECode;
...
SaccadeTime_Stim_mat =SaccadeTime_Stim;
SaccadeTime_Control_mat =SaccadeTime_Control;
...
%Use first saccade for comparison
if isempty(SaccadeAmplitude_Stim)
   Amp_Stim_FirstInTrial = NaN*ones(sum(StimTrial),1);
else
Amp_Stim_FirstInTrial=SaccadeAmplitude_Stim(:,1);
end


if isempty(SaccadeAmplitude_Control)
    Amp_Control_FirstInTrial = NaN*ones(sum(ControlTrial),1);
else
    Amp_Control_FirstInTrial=SaccadeAmplitude_Control(:,1);
end
...
NumSaccade_Stim_mat = ~isnan(SaccadeTime_Stim_mat);
SaccadeTime_Stim_mat_sel =arrayfun(@(x) x>=StimDur(1) & x<=StimDur(2),SaccadeTime_Stim_mat);
NumSaccade_Stim_In_mat = NumSaccade_Stim_mat.*SaccadeTime_Stim_mat_sel;
NumSaccade_Stim_In_mat=sum(NumSaccade_Stim_In_mat,2);


NumSaccade_Control_mat = ~isnan(SaccadeTime_Control_mat);
SaccadeTime_Control_mat_sel =arrayfun(@(x) x>=StimDur(1) & x<=StimDur(2),SaccadeTime_Control_mat);
NumSaccade_Control_In_mat = NumSaccade_Control_mat.*SaccadeTime_Control_mat_sel;
NumSaccade_Control_In_mat=sum(NumSaccade_Control_In_mat,2);

%SaccadeAngle

if isempty(SaccadeAngle)
    SaccadeAmp_Stim=NaN*ones(sum(StimTrial),1);
else
SaccadeAmp_Stim = SaccadeAngle(StimTrial,:);
end

if isempty(SaccadeAngle)
    SaccadeAmp_Control=NaN*ones(sum(ControlTrial),1);
else
...
%Use first saccade for comparison
if isempty(SaccadeAmp_Stim)
    Angle_Stim_FirstInTrial=NaN*ones(sum(StimTrial),1);
else
    Angle_Stim_FirstInTrial=SaccadeAmp_Stim(:,1);
end

if isempty(SaccadeAmp_Control)
    Angle_Control_FirstInTrial=NaN*ones(sum(ControlTrial),1);
else
    Angle_Control_FirstInTrial=SaccadeAmp_Control(:,1);
end

%Save this for comparison between VideoFreeView and FixStim and GapFixStim
AngleAmpEachTrial=[Angle_Stim_FirstInTrial,Amp_Stim_FirstInTrial];

AngleAmpEachTrial_Control=[Angle_Control_FirstInTrial,Amp_Control_FirstInTrial];
...
Angle_Stim = SaccadeAmp_Stim;
Amp_Stim = SaccadeAmplitude_Stim_line;

Angle_Control = SaccadeAmp_Control;
Amp_Control = SaccadeAmplitude_Control_line;

theta_Stim = Angle_Stim*pi/180;
theta_Control = Angle_Control*pi/180;

Angle_Stim_mean = mean(Angle_Stim );
Angle_Stim_sem = std(Angle_Stim )/sqrt(length(Angle_Stim ));
disp('Angle_Stim_mean:')
disp(Angle_Stim_mean)
disp('Angle_Stim_sem:')
disp(Angle_Stim_sem)
disp('N of stim saccades:')
disp(length(Angle_Stim))


%figure, polarplot(theta_Stim, Amp_Stim);
%figure, histogram2Polar(theta_Stim, Amp_Stim, 10, 'ThetaTicks', 0:45:360, 'thetaZeroLocation', 'right', 'edgeColor', 'k', 'Normalization', 'pdf');
 
%Stats
[h,p]=ttest2(NumSaccade_Stim_In_mat,NumSaccade_Control_In_mat);

N_Total_Stim = length(Angle_Stim);
N_Total_Control = length(Angle_Control);

SacFreq_Total_Stim = length(Angle_Stim)/NumberStimTrial*10;%in saccade/s
SacFreq_Total_Control = length(Angle_Control)/NumberControlTrial*10; % in saccade/s
...
NumberOfAngle_Stim_Count = SaccadeVectorTuning.DataNum;
...
NumberOfAngle_Control=SaccadeVectorTuning.DataNum/sum(SaccadeVectorTuning.DataNum)*100;%Change into proportion of saccades
 NumberOfAngle_Control_Count = SaccadeVectorTuning.DataNum;
...
%{
    ...
    clSAC = length(SaccadeAmp_Stim(SaccadeAmp_Stim > 181 & SaccadeAmp_Stim < 359));
    ilSAC = length(SaccadeAmp_Stim(SaccadeAmp_Stim > 0 & SaccadeAmp_Stim < 180));
...
quiver(X_Comp_Stim,Y_Comp_Stim,'autoscale','off','color','r','LineWidth',3);
title('Stim')
set(gca,'LineWidth',3,'FontSize',20,'FontWeight','Bold');
box off
...
if PlotExample
    ...
    figtitlestr{FigureIndex}='Example';
    fig{FigureIndex}=PrepareFigure(FigureStartNum,'w',[120,100, 600,300],'Name',figtitlestr{FigureIndex});
    
    subplot(1,2,1)
    quiver(SaccadeStartPoint_X_Stim_Line,SaccadeStartPoint_Y_Stim_Line,X_Comp_Stim,Y_Comp_Stim,'AutoScale', 'off','Color','r','LineWidth',3);
    xlim([-40,40]);
    ylim([-40,40]);
    subplot(1,2,2)
    quiver(SaccadeStartPoint_X_Control_Line,SaccadeStartPoint_Y_Control_Line,X_Comp_Control,Y_Comp_Control,'AutoScale', 'off','Color','k','LineWidth',3);
    xlim([-40,40]);
    ylim([-40,40]);
    % Add scale bar
...
if exist(OutputFileName)
    ...
    if TaskCode == VIDEO_FREEVIEW_TASK
        if isfield(OutputData,'FreeViewBehavior')
            ExistFlag=1;
        end
    elseif TaskCode == FIXSTIM_TASK
        if isfield(OutputData,'FixViewBehavior')
            ExistFlag=1;
        end

    elseif TaskCode == ENDFIXSTIM_TASK
        if isfield(OutputData,'GapFixViewBehavior')
            ExistFlag=1;
        end
...
elseif TaskCode == FIXSTIM_TASK
        OutputData_New.FixViewBehavior.TrialType='GoodTrials';
elseif TaskCode == ENDFIXSTIM_TASK
        OutputData_New.GapFixViewBehavior.TrialType='GoodTrials';
end
...
NameStr={'PropClSac','p_prop','NumberOfAngle_Control','NumberOfAngle_Stim',...
    ...
    'theta_Stim','Amp_Stim','theta_Control','Amp_Control','PrefVectorNum_Stim',... 
    'PrefVectorNum_Control','PrefVector_AmpNum_Stim','AngleAmpEachTrial','AngleAmpEachTrial_Control',...
    'NSac_Total_Stim','NSac_Total_Control' ,'SacFreq_Total_Stim' ,'SacFreq_Total_Control' ,...
    'SaccadeFreqStim_Sum','SaccadeFreqControl_Sum'


};

DataLib={PropClSac,p_prop,NumberOfAngle_Control_Count,NumberOfAngle_Stim_Count,...
    TimeSeq,SaccadeCountStim_Sum,SaccadeCountControl_Sum,...
    theta_Stim,Amp_Stim,theta_Control,Amp_Control,PrefVectorNum_Stim,...
    PrefVectorNum_Stim,PrefVector_AmpNum_Stim,AngleAmpEachTrial,AngleAmpEachTrial_Control,...
    N_Total_Stim,N_Total_Control,SacFreq_Total_Stim,SacFreq_Total_Control,...
    SaccadeFreqStim_Sum,SaccadeFreqControl_Sum
...
elseif TaskCode == FIXSTIM_TASK
    OutputData_New.FixViewBehavior.Task=TaskType;
    OutputData_New.FixViewBehavior.TaskCode=TaskCode;
    OutputData_New.FixViewBehavior.DataStamp=DataStamp;

elseif TaskCode == ENDFIXSTIM_TASK
    OutputData_New.GapFixViewBehavior.Task=TaskType;
    OutputData_New.GapFixViewBehavior.TaskCode=TaskCode;
    OutputData_New.GapFixViewBehavior.DataStamp=DataStamp;
end
...
if ExistFlag
%Compare the old one with the new one
Task_Old=OutputData.FixViewBehavior.TaskCode;

     if Task_Old~=TaskCode
        %Not the same task, add the new dataset into the old one
        OutputData.FixViewBehavior(numel(OutputData.FixViewBehavior)+1)=OutputData_New.FixViewBehavior;
     else
         %If the same task,the same protocol,replace the old one with the new one
         
         OutputData.FixViewBehavior=OutputData_New.FixViewBehavior;
...
elseif TaskCode == ENDFIXSTIM_TASK
   if ExistFlag
       %Compare the old one with the new one
        Task_Old=OutputData.GapFixViewBehavior.TaskCode;

     if Task_Old~=TaskCode
        %Not the same task, add the new dataset into the old one
        OutputData.GapFixViewBehavior(numel(OutputData.GapFixViewBehavior)+1)=OutputData_New.GapFixViewBehavior;
     else
         %If the same task,the same protocol,replace the old one with the new one
         
         OutputData.GapFixViewBehavior=OutputData_New.GapFixViewBehavior;
