function FreeViewEyeMovement_txtexport(Data, StartTrial, EndTrial,ShowFigureFlag,OutputFlag);
ECode;
...
%Average amplitude
SaccadeAmplitude_Ave  = nanmean(SaccadeAmplitude_Stim_line);
...
OutPath=strcat(BasicDirectory,'Results',BasicDirectory(end),'FreeViewBehavior_txt',BasicDirectory(end));%,OutputFolerName);
if ~exist(OutPath)
    mkdir(OutPath)
end
cd(OutPath);
...
%%%%%%%%%%%%%%%%%%%%%%%  Batch Output %%%%%%%%%%%%%%%%%
   ...
   % outfile = [OutPath 'ElectricalStimBehavior.dat'];
    printHead = 0;
    ...
    buff = sprintf(sprint_txt, [FileName '_' num2str(ChannelNum)],PrefVectorNum_Stim, SaccadeAmplitude_Ave, p_prop);
    
    fid = fopen(outfile, 'a');
    if (printHead)
        fprintf(fid, 'FILE\t   PreferVector, Amplitude, p\n   ');
        fprintf(fid, '\r\n');
    end
