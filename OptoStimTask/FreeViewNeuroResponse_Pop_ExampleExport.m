function FreeViewNeuroResponse_Pop_ExampleExport(Data, StartTrial, EndTrial,ShowFigureFlag,OutputFlag)
ECode;
...
%ExcludeChanel=[8,10,12,13,15,18,21,23,25,26,28,32];
ExcludeChanel =[];
...
Total_SC_Stim(spk) = mean(PSTH_Stim,'all');
Total_SC_Control(spk) = mean(PSTH_Control,'all');
...
Sig_m_fast = ResponseLatency<=10 & Sig_m;
Sig_m_slow = ResponseLatency> 10 & Sig_m;
...
%Remove the firing rate < 1
SelectedChannel = Total_SC_Stim > 1;
...
SignificanceChannel = SignificanceChannel(SelectedChannel);
SigFastExciteChannel = SigFastExciteChannel(SelectedChannel);
SigSlowExciteChannel = SigSlowExciteChannel(SelectedChannel);
SigInhibitChannel = SigInhibitChannel(SelectedChannel);
SpikeChannelWhole = SpikeChannelWhole(SelectedChannel);
ResponseLatency_store = ResponseLatency_store(SelectedChannel);


PSTH_Stim_Mean_norm2=PSTH_Stim_Mean_norm2(SelectedChannel,:);
PSTH_Stim_Mean_norm1=PSTH_Stim_Mean_norm1(SelectedChannel,:);
...
psth_use = PSTH_Stim_Mean_norm1;

%Test
%{
ChannelFOS=14:20;
ChannelSW = 4:10;
tmp = psth_use(ChannelFOS,:);
psth_use(ChannelFOS,:)=psth_use(ChannelSW,:);
psth_use(ChannelSW,:) = tmp;
tmp2=psth_use(13,:);
psth_use(13,:)=psth_use(24,:);
psth_use(24,:) = tmp2;
%}
...
if max(ChannelIDWhole)>SepChannel
    %Separate into 2
    PSTH_Stim_Mean_norm_First = psth_use(ChannelIDWhole<=SepChannel,:);
    PSTH_Stim_Mean_norm_Second = psth_use(ChannelIDWhole>SepChannel,:);
...
SignificanceChannel_first = SigFastChannel(ChannelIDWhole<=SepChannel);
SignificanceChannel_second = SigFastChannel(ChannelIDWhole>SepChannel);



ModType_first = ModType(ChannelIDWhole<=SepChannel);
ModType_second = ModType(ChannelIDWhole>SepChannel);
...
plot(time_plot(flipud(SignificanceChannel_first==1& ModType_first ==1),:)',psth_first(flipud(SignificanceChannel_first==1& ModType_first ==1),:)','-r','LineWidth',3);
plot(time_plot(flipud(SignificanceChannel_first==1& ModType_first ==-1),:)',psth_first(flipud(SignificanceChannel_first==1& ModType_first ==-1),:)','-b','LineWidth',3);
...
%plot(time_plot(flipud(SigFastChannel==1),:)',psth(flipud(SigFastChannel==1),:)','-r','LineWidth',3);
plot(time_plot(flipud(SignificanceChannel_second==1& ModType_second ==1),:)',psth_second(flipud(SignificanceChannel_second==1& ModType_second ==1),:)','-r','LineWidth',3);
plot(time_plot(flipud(SignificanceChannel_second==1& ModType_second ==-1),:)',psth_second(flipud(SignificanceChannel_second==1& ModType_second ==-1),:)','-b','LineWidth',3);
...
%}
OutputFigureName_Final=strcat(OutputFigureName,figtitlestr{1},'.pdf');
saveas(fig{1},OutputFigureName_Final);
