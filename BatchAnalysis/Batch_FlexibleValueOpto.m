function Batch_FlexibleValueOpto(Data,  StartFile, EndFile,ShowFigureFlag,OutputFlag);
...
for i=1:length(FilesName)
  ...
  for j = 1:length(FilesName{i})
     ...
     if isfield(OutputData,'StableValue')
        ...
        Data =OutputData.FlexibleValue.DataStamp;
...
%Distribution
SigLoc_Taged = (Diff_TgtOn|Diff_SacOn)&SM_p<0.05;
NonSigLoc_Taged = (~(Diff_TgtOn|Diff_SacOn))&SM_p<0.05;
Prop_Sig_Taged = sum(SigLoc_Taged)/sum(SM_p<0.05);
Prop_NonSig_Taged = sum(NonSigLoc_Taged)/sum(SM_p<0.05);

SigLoc_NonTaged = (Diff_TgtOn|Diff_SacOn)&SM_p>0.05;
NonSigLoc_NonTaged = (~(Diff_TgtOn|Diff_SacOn))&SM_p>0.05;
Prop_Sig_NonTaged = sum(SigLoc_NonTaged)/sum(SM_p>0.05);
Prop_NonSig_NonTaged = sum(NonSigLoc_NonTaged)/sum(SM_p>0.05);
