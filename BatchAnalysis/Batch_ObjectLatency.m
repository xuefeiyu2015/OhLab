function Batch_ObjectLatency(Data,  StartFile, EndFile,ShowFigureFlag,OutputFlag);
...
for i=1:length(FilesName)
    for j = 1:length(FilesName{i})
     ...
     Task(index)=OutputData.ObjectLatency.TaskCode;
     TaskName{index}=OutputData.ObjectLatency.Task;

     FilesNameAll{index}=FilesName_Each{i};
     
     Data =OutputData.ObjectLatency.DataStamp;

    % PSTH_Tgt_RF{index} = Data('PSTH_TgtOn_RF');
    % PSTH_Sac_RF{index} = Data('PSTH_SacOn_RF');

    RF_p_v(index) = Data('p_Vis_RF');
...
annotation('textbox',[.05 .7 .3 .3],'String','XF SC during ObjectSaccadeTask','LineStyle','none');
