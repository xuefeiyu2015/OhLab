...
ChannelNumber(Ex_Fast_Sig)=ChannelNumber(Ex_Fast_Sig)-5;
fast_exc_c = ChannelNumber(Ex_Fast_Sig);
fast_exc_c(find(fast_exc_c==max(fast_exc_c),1,'last'))=23;
ChannelNumber(Ex_Fast_Sig)=fast_exc_c;
ChannelNumber(ChannelNumber<=0)=ChannelNumber(ChannelNumber<=0)+7;

%For the depth analysis
OptoChannel = 3;
RelChannel=ChannelNumber-OptoChannel;

%VProbe_FastExci_units = VProbeChannel_sel' & Ex_Fast_Sig;

%Plot the proportion of neurons in each group as a function of RelChannel
UniqueChannel = min(RelChannel):max(RelChannel);
for c = 1:length(UniqueChannel )
    
    sel = (RelChannel==UniqueChannel(c) & VProbeChannel_sel)';
    
    Fast_Exc_sel = sel & Ex_Fast_Sig;
    Slow_Exc_sel = sel & Ex_Slow_Sig;
    In_sel = sel & In_Sig;
    
    TotalNum_c = sum(sel);
    NumFastExc = sum(Fast_Exc_sel);
    NumSlowExc = sum(Slow_Exc_sel);
    NumIn = sum(In_sel);

    TotalNum_save(c)=TotalNum_c;
    TotalFastExc_save(c) = NumFastExc;
   
    Prop_FastExc(c) = NumFastExc/TotalNum_c;
    Prop_SlowExc(c) = NumSlowExc/TotalNum_c;
    Prop_In(c) = NumIn/TotalNum_c;

    Latency_c = Latency(Excitation & Sig_m & sel);
    Median_c(c) = median(Latency_c);
    if Median_c(c)>100
        Median_c(c)=80;
    end
...
upperchannels = UniqueChannel<=10;

PropTotalFastExc_upper = sum(TotalFastExc_save(upperchannels))/sum(TotalFastExc_save);


FigureIndex=1;
FigureStartNum=20;
figtitlestr{FigureIndex}='PSTH_Prop_Depth';
...
formatFig(fig{FigureIndex}, [1 2], 'nature');
plot(Prop_FastExc,UniqueChannel,'-or');
hold on
plot(Prop_SlowExc,UniqueChannel,'-om','color','#FFD700');
%plot(Prop_In,UniqueChannel,'-ob','LineWidth',3);
...
box off
grid on;
legend({'FastExc','SlowExc'});
%set(gca,'LineWidth',3,'FontWeight','Bold','FontSize',15);
xlabel('Proportion of neurons');
ylabel('Relative Channel Number');


else


plot(Prop_FastExc,UniqueChannel,'-or','LineWidth',3);
hold on
plot(Prop_SlowExc,UniqueChannel,'-om','color','#FFD700','LineWidth',3);
%plot(Prop_In,UniqueChannel,'-ob','LineWidth',3);
set(gca, 'YDir', 'reverse');
set(gca, 'XAxisLocation', 'top'); 
ylim([-4,22]);
...
figtitlestr{FigureIndex}='PSTH_Latency_Depth';
fig{FigureIndex}=PrepareFigure(FigureStartNum,'w',[50,100, 800,800],'Name',figtitlestr{FigureIndex});
...
plot(Median_c,UniqueChannel,'-or','Color','#FF8C00');
...
ylim([-4,22]);
xlim([0,100]);
xlabel('Median Latency(Excitation)');
ylabel('Relative Channel Number');

else


plot(Median_c,UniqueChannel,'-or','Color','#FF8C00','LineWidth',3);
...
OutputFigureName='FEF_Depths_Recorded';
