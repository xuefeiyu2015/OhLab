function Batch_StableValue_Opto(Data,  StartFile, EndFile,ShowFigureFlag,OutputFlag);
...
%}
         PSTH_TgtOn_Prefer_RF{index} = Data('PSTH_TgtOn_Prefer_RF_z');
        PSTH_TgtOn_nonPrefer_RF{index} = Data('PSTH_TgtOn_nonPrefer_RF_z');
        PSTH_TgtOn_Prefer_nonRF{index} = Data('PSTH_TgtOn_Prefer_nonRF_z');
        PSTH_TgtOn_nonPrefer_nonRF{index} = Data('PSTH_TgtOn_nonPrefer_nonRF_z');

        PSTH_SacOn_Prefer_RF{index} = Data('PSTH_SacOn_Prefer_RF_z');
        PSTH_SacOn_nonPrefer_RF{index} = Data('PSTH_SacOn_nonPrefer_RF_z');
        PSTH_SacOn_Prefer_nonRF{index} = Data('PSTH_SacOn_Prefer_nonRF_z');
        PSTH_SacOn_nonPrefer_nonRF{index} = Data('PSTH_SacOn_nonPrefer_nonRF_z');
...
SigDiff = SM_p<0.05 & Latency <=10 & SM>0;
NonSigDiff = SM_p>=0.05;

N_Tag = sum(SigDiff)
N_nonTag = sum(NonSigDiff)

Num_Diff_Tag = sum( SigDiff& (Diff_TgtOn|Diff_SacOn));
Num_NoDiff_Tag = sum( SigDiff& ~(Diff_TgtOn|Diff_SacOn));


Num_Diff_NoTag = sum( NonSigDiff& (Diff_TgtOn|Diff_SacOn));
Num_NoDiff_NoTag = sum( NonSigDiff& ~(Diff_TgtOn|Diff_SacOn));
...
PSTH_nonPrefer_SacOn_RF = cell2mat(PSTH_SacOn_nonPrefer_RF');

PSTH_Prefer_TgtOn_nonRF = cell2mat(PSTH_TgtOn_nonPrefer_RF');
PSTH_nonPrefer_TgtOn_nonRF = cell2mat(PSTH_TgtOn_nonPrefer_nonRF');

%For opto-tagged neurons

psth_prefer_tgtOn_RFeach = PSTH_Prefer_TgtOn_RF(SigDiff,:);
psth_prefer_sacOn_RFeach = PSTH_Prefer_SacOn_RF(SigDiff,:);

psth_prefer_tgtOn_nonRFeach = PSTH_Prefer_TgtOn_RF(NonSigDiff,:);
psth_prefer_sacOn_nonRFeach = PSTH_Prefer_SacOn_RF(NonSigDiff,:);
%{
figure
plot(psth_prefer_tgtOn_nonRFeach');
figure
plot(psth_prefer_sacOn_nonRFeach');
%}
...
%Distribution
SigLoc_Taged = (Diff_TgtOn|Diff_SacOn)&SigDiff;
NonSigLoc_Taged = (~(Diff_TgtOn|Diff_SacOn))&SigDiff;
...
SigLoc_NonTaged = (Diff_TgtOn|Diff_SacOn)&NonSigDiff ;
NonSigLoc_NonTaged = (~(Diff_TgtOn|Diff_SacOn))&NonSigDiff;
...
hold on
shadedErrorBar(PSTH_Time_TgtOn,PSTH_nonPrefer_TgtOn_nonRF_Mean,PSTH_nonPrefer_TgtOn_nonRF_Mean,'lineprops',{[0,0,1]},'transparent',1,'patchSaturation',0.3);
