function Batch_Optogenetics_FEF(Data,  StartFile, EndFile,ShowFigureFlag,OutputFlag);
...
%}
  ...
  %   PSTH_Control_Norm{index} = Data('PSTH_Control_Mean_z');
   ...
   %  ChannelNum(index) = Data('ChannelNum');
...
%     p_AS(index) = Data('Ac_p');
     ...
     ChannelNumber(index) = Data('ChannelID');
     SpikeChannel(index) = Data('SpikeChannel');
...
p_Response_interval_RF(index,:)=NaN*ones(1,4);
TgtAUC_interval_RF(index,:) = NaN*ones(1,4);
...
if isfield(OutputData,'OneDR')
    ...
    Value_Vis_nonRF_OneDR(index) =NaN;
     ...
     Value_Sac_nonRF_OneDR(index) = NaN;
        ...
        p_Tgt_RF(index) = NaN;
        p_Sac_RF(index) = NaN;
        p_preTgt_RF(index) = NaN;
        Tgt_Good_RF(index) = NaN;
        Tgt_Bad_RF(index) = NaN;
        PreTgtGood_RF(index)= NaN;
        PreTgtBad_RF(index) = NaN;

        PSTH_Prefer_TgtOn_RF{index} = NaN;
        PSTH_nonPrefer_TgtOn_RF{index} = NaN;

        PSTH_Prefer_SacOn_RF{index} = NaN;
        PSTH_nonPrefer_SacOn_RF{index} = NaN;

        PSTH_Prefer_TgtOn_nonRF{index} = NaN;
        PSTH_nonPrefer_TgtOn_nonRF{index} = NaN;

        PSTH_Prefer_SacOn_nonRF{index} = NaN;
        PSTH_nonPrefer_SacOn_nonRF{index} = NaN;


        p_preTgt_nonRF(index) = NaN;
        p_Sac_nonRF(index) = NaN;
        p_AUC_Tgt_nonRF(index) = NaN;
        p_AUC_Sac_nonRF(index) =NaN;
        p_Tgt_nonRF(index) = NaN;

        TgtAUC_RF(index) = NaN;
        SacAUC_RF(index) = NaN;

        TgtAUC_nonRF(index) =NaN;
        SacAUC_nonRF(index) = NaN;

        p_AUC_Tgt_RF(index) = NaN;
        p_AUC_Sac_RF(index) = NaN;
...
ChannelNumber=ChannelNumber(ScreenCell);

SpikeChannel=SpikeChannel(ScreenCell);

VProbeChannel_sel = SpikeChannel>=4;
...
VProbe_FastExci_units = VProbeChannel_sel' & Ex_Fast_Sig;
VProbeChannel_Fast = ChannelNumber(VProbe_FastExci_units);
Latency_FastExci_units = Latency(VProbe_FastExci_units);

VProbe_SlowExci_units = VProbeChannel_sel' & Ex_Slow_Sig;
VProbeChannel_Slow = ChannelNumber(VProbe_SlowExci_units);
Latency_SlowExci_units = Latency(VProbe_SlowExci_units);

SM_FastExci_units = SM(VProbe_FastExci_units);
SM_SlowExci_units = SM(VProbe_SlowExci_units);


Latency_Exci_units = Latency(Excitation & Sig_m);
Latency_Inh_units = Latency(Inhibition & Sig_m);

Latency_Exci_units_Mean = mean(Latency_Exci_units);
Latency_Exci_units_Sem = std(Latency_Exci_units)/sqrt(length(Latency_Exci_units));

Latency_Inh_units_Mean = mean(Latency_Inh_units);
Latency_Inh_units_Sem = std(Latency_Inh_units)/sqrt(length(Latency_Inh_units));

 
[h,p_Latency_ExIn] = ttest2(Latency_Exci_units,Latency_Inh_units)

%keyboard

%{
figure
plot(Latency_FastExci_units,SM_FastExci_units,'or');
hold on
plot(Latency_SlowExci_units,SM_SlowExci_units,'om');

figure
plot(Latency_FastExci_units,VProbeChannel_Fast,'or')
hold on
plot(Latency_SlowExci_units,VProbeChannel_Slow,'om')
...
%Find memory saccade examples
%{
FastEx = Sig_m & Fast_M & Excitation;
Vis = CellType==1;
VisMotor = CellType==2;
Motor = CellType == 3;

FilesNameAll_Vis = FilesNameAll(FastEx & Vis,:);
ChannelScreened_Vis = ChannelScreened(FastEx & Vis);

FilesNameAll_VisMotor = FilesNameAll(FastEx & VisMotor,:);
ChannelScreened_VisMotor = ChannelScreened(FastEx & VisMotor);

FilesNameAll_Motor = FilesNameAll(FastEx & Motor,:);
ChannelScreened_Motor = ChannelScreened(FastEx & Motor);
%
keyboard
%}
%figure
...
SlowInhibitionFile = FilesNameScreened(Inhibition & Sig_m & Latency>10)';
SlowInhibitionChannelScreened = ChannelScreened(Inhibition & Sig_m & Latency> 10);

SlowExcFile = FilesNameScreened(Excitation & Sig_m & Latency>10)';
SlowExcChannelScreened = ChannelScreened(Excitation & Sig_m & Latency> 10);
...
%{
 %Save the FEF stimulation file
    ...
    fileName1 = fullfile(DataPath, 'FEF_OpticalStimulation.mat');
    save(fileName1, 'PSTH_Use');

    fileName2 = fullfile(DataPath, 'FEF_OpticalStimulationModuleType.mat');
    save(fileName2, 'ModuType');
%}
...
%Save the FEF stimulation file
    ...
    fileName3 = fullfile(DataPath, 'FEF_MemorySaccadeFastActivation_VisOnly_Mean.mat');
    tmp3 =PSTH_Tgt_Mean{1,1} ;
    save(fileName3, 'tmp3');

    fileName4 = fullfile(DataPath, 'FEF_MemorySaccadeFastActivation_VisOnly_Sem.mat');
    tmp4 =PSTH_Tgt_Sem{1,1} ;
    save(fileName4, 'tmp4');

    fileName5 = fullfile(DataPath, 'FEF_MemorySaccadeFastActivation_VisMove_Mean.mat');
    tmp5 =PSTH_Tgt_Mean{1,2} ;
    save(fileName5, 'tmp5');

    fileName6 = fullfile(DataPath, 'FEF_MemorySaccadeFastActivation_VisMove_Sem.mat');
    tmp6 =PSTH_Tgt_Sem{1,2} ;
    save(fileName6, 'tmp6');

    fileName7 = fullfile(DataPath, 'FEF_MemorySaccadeFastActivation_Move_Mean.mat');
    tmp7 =PSTH_Tgt_Mean{1,3} ;
    save(fileName7, 'tmp7');

    fileName8 = fullfile(DataPath, 'FEF_MemorySaccadeFastActivation_Move_Sem.mat');
    tmp8 =PSTH_Tgt_Sem{1,3} ;
    save(fileName8, 'tmp8');
...
FigureIndex = FigureIndex+1;
 ...
 figtitlestr{FigureIndex}='ModulationStrengthBar_Raw';
   fig{FigureIndex}=PrepareFigure(FigureStartNum,'w',[50,100, 600,400],'Name',figtitlestr{FigureIndex});
        
   b = bar([1,2,3],MS_Mean_Group(1:3));
   hold on
   errorbar([1,2,3],MS_Mean_Group(1:3),MS_Sem_Group(1:3))

%}

    
%Distribution of modulation strength between fast excitation and flow
%excitation and inhibition

Latency_Fast = Latency(ModuType==1);
Latency_Slow = Latency(ModuType==2);
Latency_Inh = Latency(ModuType==3);

SM_Fast = SM(ModuType==1);
SM_Slow = SM(ModuType==2);
SM_Inh = abs(SM(ModuType==3));

SM_bin = [0:10:max(abs(SM))];

FigureStartNum = FigureStartNum+1;
FigureIndex = FigureIndex+1;

 figtitlestr{FigureIndex}='ModulationStrengthDistribution';
   fig{FigureIndex}=PrepareFigure(FigureStartNum,'w',[50,100, 600,400],'Name',figtitlestr{FigureIndex});
subplot(2,1,1)
histogram(SM_Fast,SM_bin,'Facecolor',ColorOrder(1,:),'LineWidth',0.5);
%set(gca,'XScale','log');
hold on
histogram(SM_Slow,SM_bin,'Facecolor',ColorOrder(2,:),'LineWidth',0.5);
box off
%set(gca,'XScale','log');
...
%set(gca,'XScale','log');
hold on
histogram(SM_Inh,SM_bin,'Facecolor',ColorOrder(3,:),'LineWidth',0.5);
box off
%set(gca,'XScale','log');
xlabel('Modulation Strength');
ylabel('Number of Neurons');
...
Latency_Fast_Mean = mean(Latency_Fast);
Latency_Fast_Sem = std(Latency_Fast)/sqrt(length(Latency_Fast));

Latency_Slow_Mean = mean(Latency_Slow);
Latency_Slow_Sem = std(Latency_Slow)/sqrt(length(Latency_Slow));

Latency_Inh_Mean = mean(Latency_Inh);
Latency_Inh_Sem = std(Latency_Inh)/sqrt(length(Latency_Inh));

%Compare latency fast v.s. slow v.s. inh
Latency_test = Latency(ModuType<=3 & ModuType>=1);
ModuType_test = ModuType(ModuType<=3 & ModuType>=1);

[p_Latency_threetypes, tbl, stats] = kruskalwallis(Latency_test, ModuType_test,'off');

[c_latency_threetypes,~,H,GNAMES]= multcompare(stats,'display','off');
c_latency_threetypes



SM_Fast_Mean = mean(SM_Fast);
SM_Fast_Sem = std(SM_Fast)/sqrt(length(SM_Fast));

SM_Slow_Mean = mean(SM_Slow);
SM_Slow_Sem = std(SM_Slow)/sqrt(length(SM_Slow));


SM_Inh_Mean = mean(SM_Inh);
SM_Inh_Sem = std(SM_Inh)/sqrt(length(SM_Inh));

SM_test = SM(ModuType<=3 & ModuType>=1);

[p_SM_threetypes, tbl, stats] = kruskalwallis(SM_test, ModuType_test,'off');

[c_SM_threetypes,~,H,GNAMES]= multcompare(stats,'display','off');
c_SM_threetypes
...
FigureIndex = FigureIndex+1;
 ...
 figtitlestr{FigureIndex}='LatencyBar';
   fig{FigureIndex}=PrepareFigure(FigureStartNum,'w',[50,100, 600,400],'Name',figtitlestr{FigureIndex});
subplot(2,1,1)
bar([1,2],[Latency_Fast_Mean,Latency_Slow_Mean])
hold on
errorbar([1,2],[Latency_Fast_Mean,Latency_Slow_Mean],[Latency_Fast_Sem,Latency_Slow_Sem],'sk');

subplot(2,1,2)
bar([1,2],[Latency_Fast_Mean,Latency_Inh_Mean])
hold on
errorbar([1,2],[Latency_Fast_Mean,Latency_Inh_Mean],[Latency_Fast_Sem,Latency_Inh_Sem],'sk');

FigureStartNum = FigureStartNum+1;
FigureIndex = FigureIndex+1;

 figtitlestr{FigureIndex}='ModulationStrengthBar';
   fig{FigureIndex}=PrepareFigure(FigureStartNum,'w',[50,100, 600,400],'Name',figtitlestr{FigureIndex});
subplot(2,1,1)
bar([1,2],[SM_Fast_Mean,SM_Slow_Mean])
hold on
errorbar([1,2],[SM_Fast_Mean,SM_Slow_Mean],[SM_Fast_Sem,SM_Slow_Sem],'sk');

subplot(2,1,2)
bar([1,2],[SM_Fast_Mean,SM_Inh_Mean])
hold on
errorbar([1,2],[SM_Fast_Mean,SM_Inh_Mean],[SM_Fast_Sem,SM_Inh_Sem],'sk');
...
OutputFigureName='FEF_VideoView_Neural_';
