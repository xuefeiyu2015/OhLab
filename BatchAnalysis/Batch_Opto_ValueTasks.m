function Batch_Opto_ValueTasks(Data,  StartFile, EndFile,ShowFigureFlag,OutputFlag);
...
AnalysisSV = 1;%For stable  value delay saccade
AnalysisOneDR = 1;%For  one direction reward saccade
 ...
 % ModuType = ones(size(ModuType));%For a test
    for i = 1:length(UniqueMol)
        for j = 1:length(CellTypeUnique)
            ...
            sel = ModuType == UniqueMol(i) & CellType == CellTypeUnique(j) & MemorySaccade_Sel;
...
%}
      ...
      % SigVal = (p_Tgt_RF_stable<0.05)  & p_Vis & sel;
       SigVal = (p_Tgt_RF_stable<0.05)  & sel;
     % SigVal = (p_Tgt_RF_stable<0.05|p_Sac_RF_stable<0.05)& sel ;
   %   NonSigVal = ~(p_Tgt_RF_stable<0.05|p_Sac_RF_stable<0.05)& sel ;
 %  NonSigVal = ~(p_Tgt_RF_stable<0.05)& p_Vis & sel;
 NonSigVal = ~(p_Tgt_RF_stable<0.05) & sel;

    %  SigVal = SigVis; %For visualization, will change back later

       % SigVal_nonRF = p_Tgt_nonRF_stable<0.05  & SigVis;
    %   SigVal_nonRF = (p_Tgt_nonRF_stable<0.05)& p_Vis_nonRF & sel ;
    %   NonSigVal_nonRF = ~(p_Tgt_nonRF_stable<0.05)& p_Vis_nonRF & sel ;

    SigVal_nonRF = (p_Tgt_nonRF_stable<0.05)& sel ;
     NonSigVal_nonRF = ~(p_Tgt_nonRF_stable<0.05) & sel ;
     ...
     GoodPrefer_RF = (Value_Vis_RF==1) & SigVal;


       
      % GoodPrefer_nonRF = (AUC_Stable_nonRF>0.5)' & SigVal;
      GoodPrefer_nonRF = (Value_Vis_nonRF==1) & SigVal;

      % BadPrefer_RF = (AUC_Stable_RF<0.5)' & SigVal;
      % BadPrefer_nonRF = (AUC_Stable_nonRF<0.5)' & SigVal;
      BadPrefer_RF = (Value_Vis_RF==-1)& SigVal;
      BadPrefer_nonRF = (Value_Vis_nonRF==-1) & SigVal;
   ...
   %    Value_Vis_RF=AUC_Stable_RF>0.5;
         ...
         %Cell Type Counts
         TotalCellCount(i)=sum(sel);

         ValueCellCount(i) = sum(SigVal);
         NonValueCellCount(i) = sum(NonSigVal);

      %   PropValueCell(i) =ValueCellCount(i) /sum(SigVis);
      PropValueCell(i) =ValueCellCount(i) /sum(sel)*100;


        GoodPref_Vis_RF = Value_Vis_RF == 1 & SigVal;
         GoodPref_Vis_RF_Count(i) = sum(GoodPref_Vis_RF);
         GoodPref_Vis_RF_Prop(i) = sum(GoodPref_Vis_RF)/ValueCellCount(i)*100;


          BadPref_Vis_RF = Value_Vis_RF == -1 & SigVal;
         BadPref_Vis_RF_Count(i) = sum(BadPref_Vis_RF);
         BadPref_Vis_RF_Prop(i) = sum(BadPref_Vis_RF)/ValueCellCount(i)*100;
...
ValueInfo_Summary(i,:)=[TotalCellCount(i),ValueCellCount(i),NonValueCellCount(i), PropValueCell(i),GoodPref_Vis_RF_Count(i),BadPref_Vis_RF_Count(i),GoodPref_Vis_RF_Prop(i), BadPref_Vis_RF_Prop(i)];
...
%}
       ...
       %     PSTH_SacOn_Good_nonRF_stable_Mo{i} = cell2mat(PSTH_SacOn_Good_nonRF_stable(SigVal)');
        ...
        PSTH_TgtOn_PreferGood_Good_RF_stable_sel= cell2mat(PSTH_TgtOn_Good_RF_stable(GoodPref_Vis_RF)');
        PSTH_TgtOn_PreferGood_Bad_RF_stable_sel= cell2mat(PSTH_TgtOn_Bad_RF_stable(GoodPref_Vis_RF)');

        PSTH_TgtOn_PreferBad_Good_RF_stable_sel= cell2mat(PSTH_TgtOn_Good_RF_stable(BadPref_Vis_RF)');
        PSTH_TgtOn_PreferBad_Bad_RF_stable_sel= cell2mat(PSTH_TgtOn_Bad_RF_stable(BadPref_Vis_RF)');

        PSTH_TgtOn_GoodPrefer_Good_RF_stable_Mean(i,:)=nanmean( PSTH_TgtOn_PreferGood_Good_RF_stable_sel,1);
        PSTH_TgtOn_GoodPrefer_Bad_RF_stable_Mean(i,:)=nanmean( PSTH_TgtOn_PreferGood_Bad_RF_stable_sel,1);

        PSTH_TgtOn_GoodPrefer_Good_RF_stable_Sem(i,:)=nanstd( PSTH_TgtOn_PreferGood_Good_RF_stable_sel,1)/(sqrt(sum(GoodPref_Vis_RF))*2);
        PSTH_TgtOn_GoodPrefer_Bad_RF_stable_Sem(i,:)=nanstd( PSTH_TgtOn_PreferGood_Bad_RF_stable_sel,1)/(sqrt(sum(GoodPref_Vis_RF))*2);

         PSTH_TgtOn_BadPrefer_Good_RF_stable_Mean(i,:)=nanmean( PSTH_TgtOn_PreferBad_Good_RF_stable_sel,1);
        PSTH_TgtOn_BadPrefer_Bad_RF_stable_Mean(i,:)=nanmean( PSTH_TgtOn_PreferBad_Bad_RF_stable_sel,1);

        PSTH_TgtOn_BadPrefer_Good_RF_stable_Sem(i,:)=nanstd( PSTH_TgtOn_PreferBad_Good_RF_stable_sel,1)/(sqrt(sum(BadPref_Vis_RF))*2);
        PSTH_TgtOn_BadPrefer_Bad_RF_stable_Sem(i,:)=nanstd( PSTH_TgtOn_PreferBad_Bad_RF_stable_sel,1)/(sqrt(sum(BadPref_Vis_RF))*2);

        %{
        figure
        subplot(1,2,1)
        errorbar(PSTH_Time_TgtOn_stable,PSTH_TgtOn_GoodPrefer_Good_RF_stable_Mean(i,:),PSTH_TgtOn_GoodPrefer_Good_RF_stable_Sem(i,:),'-r');
        hold on
         errorbar(PSTH_Time_TgtOn_stable,PSTH_TgtOn_GoodPrefer_Bad_RF_stable_Mean(i,:),PSTH_TgtOn_GoodPrefer_Bad_RF_stable_Sem(i,:),'-b');

          subplot(1,2,2)
        errorbar(PSTH_Time_TgtOn_stable,PSTH_TgtOn_BadPrefer_Good_RF_stable_Mean(i,:),PSTH_TgtOn_BadPrefer_Good_RF_stable_Sem(i,:),'-r');
        hold on
         errorbar(PSTH_Time_TgtOn_stable,PSTH_TgtOn_BadPrefer_Bad_RF_stable_Mean(i,:),PSTH_TgtOn_BadPrefer_Bad_RF_stable_Sem(i,:),'-b');
        %}
...
%}
disp('Value Information Summary for stable value task');
ValueInfo_Summary
...
%}
         ...
         % NonSigVall_All = ~SigVal_All;
          ...
          SigVal = (p_Tgt_RF<0.05 | p_preTgt_RF < 0.05) & sel ;
          NonSigVal = ~(p_Tgt_RF<0.05 | p_preTgt_RF < 0.05)& sel ;

          SigVal_NonRF = (p_Tgt_nonRF<0.05 | p_preTgt_nonRF < 0.05) & sel ;
          NonSigVal_NonRF = ~(p_Tgt_nonRF<0.05| p_preTgt_nonRF < 0.05)& sel ;


         %% Cell Type Counts
         TotalCell_OneDR(i) = sum(sel);

         ValueCellCount_oneDR(i) = sum( SigVal );

         NonValueCellCount_oneDR(i) = sum(NonSigVal);
         ...
         %}
         GoodPrefer_RF = Value_Vis_RF_OneDR == 1 & SigVal;
         GoodPrefer_NonRF =Value_Vis_nonRF_OneDR == 1 & SigVal_NonRF;

         BadPrefer_RF =Value_Vis_RF_OneDR == -1 & SigVal;
         BadPrefer_NonRF = Value_Vis_RF_OneDR == -1 & SigVal_NonRF;
         ...
         %}
         GoodPreferCount(i) = sum(GoodPrefer_RF);
         BadPreferCount(i) = sum(BadPrefer_RF);

          GoodPreferProp(i) = sum(GoodPrefer_RF)/sum( SigVal);
          BadPreferProp(i) = sum(BadPrefer_RF)/sum(SigVal);


         ValueSummary_OneDR(i,:)=[ TotalCell_OneDR(i) ,ValueCellCount_oneDR(i),NonValueCellCount_oneDR(i), PropValueCell_OneDR(i)*100,GoodPreferCount(i),BadPreferCount(i), GoodPreferProp(i)*100,BadPreferProp(i)*100];




        PSTH_TgtOn_PreferGood_Good_RF_sel = cell2mat(PSTH_Good_TgtOn_RF(GoodPrefer_RF)');
         PSTH_TgtOn_PreferGood_Bad_RF_sel = cell2mat(PSTH_Bad_TgtOn_RF(GoodPrefer_RF)');

          PSTH_TgtOn_PreferBad_Good_RF_sel = cell2mat(PSTH_Good_TgtOn_RF(BadPrefer_RF)');
         PSTH_TgtOn_PreferBad_Bad_RF_sel = cell2mat(PSTH_Bad_TgtOn_RF(BadPrefer_RF)');


        PSTH_TgtOn_PreferGood_Good_RF_Mean(i,:)=nanmean(PSTH_TgtOn_PreferGood_Good_RF_sel,1);
         PSTH_TgtOn_PreferGood_Bad_RF_Mean(i,:)=nanmean(PSTH_TgtOn_PreferGood_Bad_RF_sel,1);

          PSTH_TgtOn_PreferBad_Good_RF_Mean(i,:)=nanmean(PSTH_TgtOn_PreferBad_Good_RF_sel,1);
         PSTH_TgtOn_PreferBad_Bad_RF_Mean(i,:)=nanmean(PSTH_TgtOn_PreferBad_Bad_RF_sel,1);
        

         PSTH_TgtOn_PreferGood_Good_RF_Sem(i,:)=nanstd(PSTH_TgtOn_PreferGood_Good_RF_sel,[],1)/(sqrt(sum(GoodPrefer_RF))*2);
         PSTH_TgtOn_PreferGood_Bad_RF_Sem(i,:)=nanstd(PSTH_TgtOn_PreferGood_Bad_RF_sel,[],1)/(sqrt(sum(GoodPrefer_RF))*2);

          PSTH_TgtOn_PreferBad_Good_RF_Sem(i,:)=nanstd(PSTH_TgtOn_PreferBad_Good_RF_sel,[],1)/(sqrt(sum(BadPrefer_RF))*2);
         PSTH_TgtOn_PreferBad_Bad_RF_Sem(i,:)=nanstd(PSTH_TgtOn_PreferBad_Bad_RF_sel,[],1)/(sqrt(sum(BadPrefer_RF))*2);
       



     end
disp('Value Information Summary for OneDR task');
ValueSummary_OneDR
...
%ValueInfo_Summary(i,:)=[TotalCellCount(i),ValueCellCount(i),NonValueCellCount(i), PropValueCell(i),GoodPref_Vis_RF_Count(i),BadPref_Vis_RF_Count(i),GoodPref_Vis_RF_Prop(i), BadPref_Vis_RF_Prop(i)];
        ...
        for i = 1:length(UniqueMol)
            ...
            b = bar([1,2,3,4],ValueInfo_Summary(i,1:4));
            b.FaceColor='flat';
            b.CData = ColorOrder(i,:);
            xticklabels({'Total','Value','NonValue','ValueProp'});

            subplot(4,2,2*(i-1)+2);
            b = bar([1,2,3,4],ValueInfo_Summary(i,5:8));
            b.FaceColor='flat';
            b.CData = ColorOrder(i,:);
            xticklabels({'GoodPref','BadPref','GPrprop','BPrprop'});
 ...
 %% Population PSTH
    ...
    for i = 1:length(UniqueMol)
        ...
        figtitlestr{FigureIndex}=sprintf('PSTH_Contralateral_StableValueFor%s',ModulString{i});
        fig{FigureIndex}=PrepareFigure(FigureStartNum,'w',[50,100, 800,400],'Name',figtitlestr{FigureIndex});


    subplot(1,2,1)
     shadedErrorBar(PSTH_Time_TgtOn_stable,PSTH_TgtOn_GoodPrefer_Good_RF_stable_Mean(i,:),PSTH_TgtOn_GoodPrefer_Good_RF_stable_Sem(i,:),'lineprops',{[228,26,28]/255},'transparent',1,'patchSaturation',0.3)
     set(findall(gca, 'Type', 'Line'),'LineWidth',3);


    hold on
    shadedErrorBar(PSTH_Time_TgtOn_stable,PSTH_TgtOn_GoodPrefer_Bad_RF_stable_Mean(i,:),PSTH_TgtOn_GoodPrefer_Bad_RF_stable_Sem(i,:),'lineprops',{[55,126,184]/255},'transparent',1,'patchSaturation',0.3);
   % ylim([-1,5]);
   ...
   yrange =ylim;
    ...
    subplot(1,2,2)
     shadedErrorBar(PSTH_Time_TgtOn_stable,PSTH_TgtOn_BadPrefer_Good_RF_stable_Mean(i,:),PSTH_TgtOn_BadPrefer_Good_RF_stable_Sem(i,:),'lineprops',{[228,26,28]/255},'transparent',1,'patchSaturation',0.3)
     set(findall(gca, 'Type', 'Line'),'LineWidth',3);


    hold on
    shadedErrorBar(PSTH_Time_TgtOn_stable,PSTH_TgtOn_BadPrefer_Bad_RF_stable_Mean(i,:),PSTH_TgtOn_BadPrefer_Bad_RF_stable_Sem(i,:),'lineprops',{[55,126,184]/255},'transparent',1,'patchSaturation',0.3);
   % ylim([-1,5]);
yrange =ylim;
    plot([0,0],yrange,'--k','LineWidth',3);
     xlim([-300,400]);
...
%}
  ...
  if PlotFlexibleValue 
    ...
    subplot(length(UniqueMol),2,(i-1)*2+1)
     shadedErrorBar(PSTH_Time_TgtOn_flexible,PSTH_TgtOn_Prefer_RF_flexible_mean(i,:),PSTH_TgtOn_Prefer_RF_flexible_sem(i,:),'lineprops',{[77,175,74]/255},'transparent',1,'patchSaturation',0.3)
     set(findall(gca, 'Type', 'Line'),'LineWidth',3);


    hold on
    shadedErrorBar(PSTH_Time_TgtOn_flexible,PSTH_TgtOn_NonPrefer_RF_flexible_mean(i,:),PSTH_TgtOn_NonPrefer_RF_flexible_sem(i,:),'lineprops',{[152,78,163]/255},'transparent',1,'patchSaturation',0.3);
    ylim([-1,5]);
    ...
    shadedErrorBar(PSTH_Time_SacOn_flexible,PSTH_SacOn_Prefer_RF_flexible_mean(i,:),PSTH_SacOn_Prefer_RF_flexible_sem(i,:),'lineprops',{[77,175,74]/255},'transparent',1,'patchSaturation',0.3);
    hold on
    shadedErrorBar(PSTH_Time_SacOn_flexible,PSTH_SacOn_NonPrefer_RF_flexible_mean(i,:),PSTH_SacOn_NonPrefer_RF_flexible_sem(i,:),'lineprops',{[152,78,163]/255},'transparent',1,'patchSaturation',0.3);
    ylim([-1,5]);
   set(findall(gca, 'Type', 'Line'),'LineWidth',3);
...
%% Counts
 ...
 fig{FigureIndex}=PrepareFigure(FigureStartNum,'w',[50,100, 500,600],'Name',figtitlestr{FigureIndex});
  ...
  for i = 1:length(UniqueMol)
            ...
            b = bar([1,2,3,4],ValueSummary_OneDR(i,1:4));
            b.FaceColor='flat';
            b.CData = ColorOrder(i,:);
            xticklabels({'Total','Value','NonValue','ValueProp'});

            subplot(4,2,2*(i-1)+2);
            b = bar([1,2,3,4],ValueSummary_OneDR(i,5:8));
            b.FaceColor='flat';
            b.CData = ColorOrder(i,:);
            xticklabels({'GoodPref','BadPref','GPrprop','BPrprop'});
...
%% For Pupulation PSTH
    ...
    for i = 1:length(UniqueMol)
        ...
        figtitlestr{FigureIndex}=sprintf('PSTH_Contralateral_OneDRFor%s',ModulString{i});
        fig{FigureIndex}=PrepareFigure(FigureStartNum,'w',[50,100, 800,400],'Name',figtitlestr{FigureIndex});
   subplot(1,2,1)
     shadedErrorBar(PSTH_Time_TgtOn_OneDR,PSTH_TgtOn_PreferGood_Good_RF_Mean(i,:),PSTH_TgtOn_PreferGood_Good_RF_Sem(i,:),'lineprops',{[44,141,90]/255},'transparent',1,'patchSaturation',0.3)
     set(findall(gca, 'Type', 'Line'),'LineWidth',3);


    hold on
    shadedErrorBar(PSTH_Time_TgtOn_OneDR, PSTH_TgtOn_PreferGood_Bad_RF_Mean(i,:), PSTH_TgtOn_PreferGood_Bad_RF_Sem(i,:),'lineprops',{[118,27,205]/255},'transparent',1,'patchSaturation',0.3);
     yrange =ylim;
    plot([0,0],yrange,'--k','LineWidth',3);
    xlim([-300,400]);
    ...
    set(gca,'LineWidth',3,'FontSize',15,'FontWeight','Bold');
     ...
     subplot(1,2,2)
     shadedErrorBar(PSTH_Time_TgtOn_OneDR,PSTH_TgtOn_PreferBad_Good_RF_Mean(i,:), PSTH_TgtOn_PreferBad_Good_RF_Sem(i,:),'lineprops',{[44,141,90]/255},'transparent',1,'patchSaturation',0.3)
     set(findall(gca, 'Type', 'Line'),'LineWidth',3);


    hold on
    shadedErrorBar(PSTH_Time_TgtOn_OneDR,PSTH_TgtOn_PreferBad_Bad_RF_Mean(i,:),  PSTH_TgtOn_PreferBad_Good_RF_Sem(i,:),'lineprops',{[118,27,205]/255},'transparent',1,'patchSaturation',0.3);
    hold on
    yrange =ylim;
    plot([0,0],yrange,'--k','LineWidth',3);
     xlim([-300,400]);
...
OutPath=strcat(BasicDirectory,'Results',BasicDirectory(end),'PopulationFigure',BasicDirectory(end));%,OutputFolerName);
if ~exist(OutPath)
    mkdir(OutPath)
end
cd(OutPath);
